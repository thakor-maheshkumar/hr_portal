    ...
    <div class="login-page">
      ...
      <form class="form login-form" method="POST" action="{{ route('register') }}">
        ...
        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
          ...
          {!! Form::text('name',$name, ['class' => 'form-control', 'required' => 'required', 'placeholder' => 'Enter your name']) !!}
          <small class="text-danger">{{ $errors->first('name') }}</small>
        </div>
        <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
          {!! Form::label('email', 'Email address') !!}
          {!! Form::email('email', $emailid, ['class' => 'form-control', 'placeholder' => 'eg: foo@bar.com']) !!}

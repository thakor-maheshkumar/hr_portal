...
Route::get('/usertopics',function(){
  return view('usertopics');
})->name('usertopics.get');

Route::get('/home', function(){
  //Start below the code customization by NP
  $auth = Auth::user();
  ...
  $topics =  Topic::select('topics.id','topics.title','topics.description','topics.per_q_mark','topics.timer','topics.show_ans','topics.amount','topics.created_at','topics.updated_at','topics.test_type')
                ->join('user_topic_mapping as utm', 'topics.id', '=', 'utm.topic_id')
                ->where('utm.user_id','=', $user_id)
                ->withCount('descriptive_questions')->QuestionCount()
                ->get();  
  //End
  //start below the code for regular              
  //$topics = Topic::all();
  //End
  
  $questions = Question::all();
  $mpt=OptionalMultiple::all();
  return view('home', compact('topics', 'questions','mpt'));
});
...
Route::group(['middleware'=> 'isadmin'], function(){
  ...
  Route::get('/answer/show/{student_id}/{topic_id}/','AllReportController@answershow');
  Route::post('answer_mark_store','AnswersController@store');
  ...
  Route::get('/admin/descriptive/scoring','QuestionsController@descriptiveScoring');

  // Optional + Multiple Routes
  Route::get('/admin/optional/multiple','QuestionsController@optionalMultiple');
  Route::post('/admin/optional/multiple/store','QuestionsController@storeOptionalMultipleQuestion');
  Route::post('/admin/optinal/multiple/update/{id}','QuestionsController@updateOptionalMultiple');
  Route::get('/admin/optional/multiple/delete/{id}','QuestionsController@deleteOptionalMultiple');
  Route::get('/optional/marksshow/{student_id}/{topic_id}','AllReportController@optionalmarksshow');
  //
  Route::post('admin/descriptive/questionsstore','QuestionsController@descriptiveQuestionStore');
  Route::post('admin/descriptive/questions/update/{id}','QuestionsController@updateDescriptiveQuestion');
  Route::get('admin/descriptive/questions/destroy/{id}','QuestionsController@destroyDescriptiveQuestion');
...
Route::get('start_quiz/question_count/{user_id}/{topic_id}','AnswersController@getQuestion');

Route::get('start_quiz/{id}/des','MainQuizController@showDescrptiveQuestion');
Route::get('start_quiz/{id}/optional','MainQuizController@showOptionalQuestion');
Route::get('start_quiz/{id}/finish', function($id){
  $auth = Auth::user();
  $topic = Topic::findOrFail($id);
  $questions = Question::where('topic_id', $id)->get();
  $count_questions = $questions->count();
  $optionalmultiple=OptionalMultiple::where('topic_id',$id)->get();
  $count_optional=$optionalmultiple->count();
  $answers = Answer::where('user_id',$auth->id)
              ->where('topic_id',$id)->get();
              
  $answerSum=Answer::where('user_id',$auth->id)->where('topic_id',$id)->get();
  $answerSumTotal=$answerSum->sum('marks'); 
  //dd($answerSumTotal);
  ...
  return view('finish', compact('ans','q','topic', 'answers', 'count_questions','count_optional','answerSumTotal'));
});
...
Route::get('admin/usertopics/','UsertopicsController@index')->name('usertopics.index');
Route::get('admin/usertopics/add','UsertopicsController@create')->name('usertopics.add');
Route::post('/admin/usertopics/insert','UsertopicsController@store')->name('usertopics.store');
Route::get('/admin/usertopics/edit/{id}','UsertopicsController@edit')->name('usertopics.edit');
Route::put('/admin/usertopics/edit/{id}','UsertopicsController@update')->name('usertopics.update');
Route::delete('/usertopics/delete/{id}','UsertopicsController@destroy')->name('usertopics.delete');
...
// route for processing payment
  ...
  Route::post('/applicationform/SaveApplication', 'ApplicationFormController@SaveApplication')->name('SaveApplication');
  Route::get('thankyou', 'ApplicationFormController@thankyou')->name('thankyou');

  Route::get('/download/{id}','ApplicationFormController@download');
